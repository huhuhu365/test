...
## 仓库与范围
- 根目录主应用是二手车管理系统，提供车辆查询、筛选、增删改、统计及 CSV 导出；前端还包含附近餐饮与游玩地点查询界面。
- 根目录 `src/` 是 Vue 前端，`backend/` 是对应的 Spring Boot API。
- `bulk-letter-vue/`、`mobile-order/`、`my-vue/`、`react-basic/`、`react-learning/`、`wang1/`、`vue-student-management/` 是其他项目目录。修改其中内容前，先检查其自身 README、依赖和脚本；不要套用主应用的技术栈与启动命令。
- `README.md` 描述主应用与启动方式，`DATABASE_PRACTICE.md` 说明数据库练习；涉及具体行为时仍需核对实现。

## 主应用技术栈
- 前端：Vue 3.5、Vite 7、JavaScript ES Modules、`@lucide/vue` 图标；具体版本约束见根目录 `package.json`，解析版本见锁文件。
...
## 命令与验证
- 以下命令仅适用于根目录主应用，在执行前确认所在目录。
- 前端开发：根目录运行 `npm run dev`；生产构建：`npm run build`；预览：`npm run preview`。
- 后端开发：在 `backend/` 运行 `mvn spring-boot:run`；后端测试：在 `backend/` 运行 `mvn test`。
- 根目录目前没有 `test` 或 `lint` 脚本，不要声称执行过不存在的检查。
...
## 避免事项
- 不手工修改 `node_modules/`、`dist/`、`backend/target/` 或缓存产物。
